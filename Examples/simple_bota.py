    ...
    if message.content.startswith(prefix):
        if command == "hello":                                    # Just an example of how formats work, it replaces the number 
            if len(message.mentions) > 0:                         # of its position in the array.
                await client.send_message(message.channel, "{0.mention} says hello to {1.mention}".format(message.author, message.mentions[0]))
            else:
                await client.send_message(message.channel, "Hello, {0.mention}!".format(message.author))
                
                
        if command == "goodbye":
            if len(message.mentions) > 0:
                await client.send_message(message.channel, "{0.mention} says goodbye to {1.mention}".format(message.author, message.mentions[0]))
            else:
                await client.send_message(message.channel, "Goodbye, {0.mention}!".format(message.author))
